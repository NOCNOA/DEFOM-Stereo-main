...
from core.defom_hw_galerkin import DEFOMStereo, autocast
...
# 若项目里用了 deterministic algorithms，评估阶段建议关闭：
try:
...
def validate_things(model, iters=32, scale_iters=8, mixed_prec=False, max_disp=192, bad_threshold=1.0):
    ...
    for val_id in tqdm(range(len(val_dataset))):
        ...
        #print("asdasd",disp_gt.shape, flush=True)
        padder = InputPadder(image1.shape, divis_by=32)
        image1, image2 = padder.pad(image1, image2)
        #torch.save(disp_gt, "disp_gt.pth")
        ...
        #print("asdas1d",disp_pr.shape, flush=True)
        assert disp_pr.shape == disp_gt.shape, (disp_pr.shape, disp_gt.shape)
        epe = torch.sum(torch.abs(disp_pr - disp_gt), dim=0)
        #disp2 = torch.load("disppr.pth")
        #save_residual_gray_png_1hw(disp_pr, disp_gt, png_path="resimap.png", vmax=50)
        #save_residual_gray_png_1hw(disp_pr, disp2, png_path="resimap2.png", vmax=50)
        epe = epe.flatten()
        ...
        if val_id < 900 or (val_id+1) % 1000 == 0:
...
def validate_eth3d(model, iters=32, scale_iters=8, mixed_prec=False):
    ...
    for val_id in tqdm(range(len(val_dataset))):
        ...
        assert disp_pr.shape == disp_gt.shape, (disp_pr.shape, disp_gt.shape)
        epe = torch.sum(torch.abs(disp_pr - disp_gt), dim=0)
        save_residual_gray_png_1hw(disp_pr, torch.zeros_like(disp_pr), png_path="resimap.png", vmax=50)
        save_residual_gray_png_1hw(disp_gt, torch.zeros_like(disp_pr), png_path="resimap2.png", vmax=50)
...
@torch.no_grad()
def validate_kitti(model, iters=32, scale_iters=8, split='2015', mixed_prec=False):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_disp', type=int, default=200, help='evaluate middlebury in detail (for different regions)')
    parser.add_argument('--mixed_precision', action='store_true', help='use mixed precision')
    parser.add_argument('--valid_iters', type=int, default=80, help='number of disparity field updates during forward pass')
    ...
    if 'kitti12' in args.datasets:
        validate_kitti(model, iters=args.valid_iters, scale_iters=args.scale_iters, split='2012', mixed_prec=use_mixed_precision)

    if 'kitti15' in args.datasets:
        validate_kitti(model, iters=args.valid_iters, scale_iters=args.scale_iters, split='2015', mixed_prec=use_mixed_precision)
