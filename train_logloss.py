...
from core.defom_cd_loss import DEFOMStereo
...
def train(args):
    ...
    while should_keep_training:
        ...
        for i_batch, data_blob in enumerate(tqdm(train_loader, initial=epoch_start_step)):
            ...
            disp_predictions, conf_prediction = model(image1, image2, iters=args.train_iters, scale_iters=args.scale_iters)
            #print("asdasd", disp_predictions)
            #print("asdass", conf_prediction)
            assert model.training

            loss, metrics = sequence_loss_withconf(disp_predictions, conf_prediction, disp_gt, valid)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_size', type=int, nargs='+', default=[320, 560], help="size of the random image crops used during training.")
    ...
    parser.add_argument('--val_freq', default=20000, type=int, help='validation frequency in terms of training steps')
    parser.add_argument('--max_disp', type=int, default=140, help="maximum disparity")
