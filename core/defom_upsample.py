...
from core.update_sele import BasicSelectiveMultiUpdateBlock ,ScaleSelectiveMultiUpdateBlock
from core.extractor import MultiBasicEncoder,  DefomEncoder, Feature
from core.corr import CorrBlock1D1
from core.utils.utils import coords_grid, upflow, get_danv2_io_size
from core.submodules import build_gwc_volume, build_concat_volume, Conv2x , ResnetBasicBlock3D, BasicConv, context_upsample, CostVolumeDisparityAttention, ChannelAttentionEnhancement, SpatialAttentionExtractor, BasicConv_IN
from core.galerkin import simple_attn_3d3
try:
...
class DEFOMStereo(nn.Module):
    def __init__(self, args):
        ...
        self.update_block = BasicSelectiveMultiUpdateBlock(self.args, hidden_dim=args.hidden_dims[0], volume = volume_dim)
        self.scale_update_block = ScaleSelectiveMultiUpdateBlock(self.args, hidden_dim=args.hidden_dims[0], volume = volume_dim)
        ...
        self.disp_att = CostVolumeDisparityAttention(d_model=volume_dim, nhead=4, dim_feedforward=volume_dim, norm_first=False, num_transformer=4, max_len=self.args.max_disp)
        #self.galerkin_attH = simple_attn_3d4(volume_dim, 4)
        self.galerkin_att = simple_attn_3d3(volume_dim, 4)
    ...
    def forward(self, image1, image2, iters=12, scale_iters=3, test_mode=False):
        ...
        with autocast(enabled=self.args.mixed_precision):
            ...
            volume_gaW = self.galerkin_att(volume)
            #torch.save(volume_gaW, "visual/volume2.pth")
            volume_dt = self.disp_att(volume)
            #torch.save(volume_dt, "visual/volume3.pth")
            volume = volume_dt + volume_gaW
            #print(f"volume: {volume.shape}", flush=True)
            #volume: torch.Size([1, 16, 184, 80, 184])
            #del volume_ga
            

        corr_fn = CorrBlock1D1(volume, coords, radius=self.args.corr_radius, num_levels=self.args.corr_levels,
        ...
        for itr in range(iters):
            ...
            if up_mask is None:
                ...
                disp_up = self.upsample_disp(disp.float(), up_mask.float(), stem_2x.float())
