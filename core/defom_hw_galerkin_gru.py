...
from core.submodules import build_gwc_volume, build_concat_volume, ResnetBasicBlock3D, BasicConv, FeatureAtt, CostVolumeDisparityAttention, ChannelAttentionEnhancement
...
class DEFOMStereo(nn.Module):
    def __init__(self, args):
        ...
        self.update_block = BasicMultiUpdateBlock(self.args, hidden_dims=args.hidden_dims, dim=volume_dim+1)
        self.scale_update_block = ScaleBasicMultiUpdateBlock(self.args, hidden_dims=args.hidden_dims, dim=volume_dim+1)
    ...
    def forward(self, image1, image2, iters=12, scale_iters=3, test_mode=False):
        ...
        for itr in range(iters):
            ...
            if itr < scale_iters:
                ...
                with autocast(enabled=self.args.mixed_precision):
                    net_list, up_mask, scale_disp = self.scale_update_block(
                        net_list,
                        inp_list,
                        corr,
                        disp,
