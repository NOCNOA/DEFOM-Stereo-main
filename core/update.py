...
class BasicMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dims=[128, 128, 128], dim = 1):
        super().__init__()
        self.args = args
        encoder_output_dim = 128
        cor_planes = args.corr_levels * (2*args.corr_radius + 1) * dim
...
class ScaleBasicMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dims=[128, 128, 128], dim = 1):
        ...
        self.args = args
        encoder_output_dim = 128
        cor_planes = len(args.scale_list) * (2*args.scale_corr_radius + 1) * dim
        self.encoder = BasicMotionEncoder(cor_planes, out_planes=encoder_output_dim)
