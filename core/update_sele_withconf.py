...
class BasicMotionEncoder(nn.Module):
    def __init__(self, args, cor_planes=8):
        ...
        self.conv = nn.Conv2d(64+256+32, 128-2, 3, padding=1)
        self.conf_occ_feat = nn.Sequential(nn.Conv2d(1, 64, kernel_size=3,padding=1),
                                       nn.GELU(),
                                       nn.Conv2d(64, 32, kernel_size=1))
    def forward(self, disp, conf_logit, corr):
        cor = F.relu(self.convc1(corr))
        cor = F.relu(self.convc2(cor))
        disp_ = F.relu(self.convd1(disp))
        disp_ = F.relu(self.convd2(disp_))
        conf_feat = self.conf_occ_feat(conf_logit)
        cor_disp = torch.cat([cor, disp_, conf_feat], dim=1)
        out = F.relu(self.conv(cor_disp))
        return torch.cat([out, disp, conf_logit], dim=1)
...
class BasicSelectiveMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dim=128, volume=16):
        ...
        self.conf_occ_update = nn.Sequential(nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, padding=1),
                                         nn.GELU(),
                                         nn.Conv2d(hidden_dim, 1, kernel_size=3, padding=1, bias=False))
    def forward(self, net, inp, corr, disp, conf, att):
        ...
        motion_features = self.encoder(disp, conf_logit, corr)
        ...
        conf_update = self.conf_occ_update(net[0])
        # scale mask to balence gradients
        mask = .25 * self.mask1(net[0])
        logit_new = conf_update + conf_logit
        conf_new = torch.sigmoid(logit_new).to(disp.dtype)

        return net, mask, delta_disp, conf_new, logit_new

class BasicSelectiveMultiUpdateBlock2(nn.Module):
    def __init__(self, args, hidden_dim=128, volume=16):
        super().__init__()
        self.args = args
        cor_planes = args.corr_levels * (2*args.corr_radius + 1) * volume #2 * 9 * 16 = 288  -> 3 * 9 * 16 = 432
    ...
    def forward(self, net, inp, corr, disp, conf, att):
        ...
        return net, mask, delta_disp, conf_new
...
class ScaleSelectiveMultiUpdateBlock(nn.Module):
    ...
    def forward(self, net, inp, corr, disp, conf, att):
        ...
        return net, mask, scale_disp, conf_new